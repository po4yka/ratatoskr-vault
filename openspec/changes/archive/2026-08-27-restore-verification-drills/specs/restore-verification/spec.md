...
byte lengths, verify the manifest signature against an explicitly configured trusted key, and validate
every parent-manifest BlobRef back to a configured trust anchor. A
missing artifact, digest/size mismatch, invalid signature, broken parent link, or failed bundle check
MUST produce failed terminal evidence and MUST NOT produce a successful restore report.
